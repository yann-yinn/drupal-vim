...
1. Contents					*TwitVim* *TwitVim-contents*
	...
	3.2. StatusNet............................: |TwitVim-StatusNet|
	...
	3.5. Using Twitter SSL API................: |TwitVim-ssl|
	     twitvim_force_ssl....................: |twitvim_force_ssl|
...
2. Introduction						*TwitVim-intro*
	...
	Since version 0.8, TwitVim supports identi.ca via OAuth. Setup for
	identi.ca is the same as for Twitter. TwitVim also supports StatusNet
	services via their Twitter-compatible APIs but not through OAuth. See
	|TwitVim-StatusNet| for information on configuring TwitVim for those
	services.
...
3. Installation						*TwitVim-install*
	...
	Twitter and identi.ca. If you intend to use TwitVim with a StatusNet
	service other than identi.ca, see |TwitVim-StatusNet|.
...
<
	...
	If you installed from the Vimball (.vba or .vmb) file, twitvim.vim should
...
<
	...
	In order to log in with Twitter/identi.ca OAuth, TwitVim needs to
	launch your web browser and bring up the Twitter authentication web
	page.
...
<
	...
	For added security, you can make TwitVim use SSL when calling the
	Twitter/identi.ca API. See |TwitVim-ssl| for instructions and
	prerequisites.

	Note: SSL is currently optional for Twitter but compulsory for
	identi.ca's OAuth handshake.


	8. Sign into Twitter with OAuth.
	...
	initiate the Twitter/identi.ca OAuth handshake. Then it'll launch your
	web browser to a Twitter or identi.ca web page that asks you to
	authorize TwitVim to use your account. On this page, sign in, if
	necessary, and then click on "Authorize app" to allow TwitVim access
	to your account.

	Twitter/identi.ca will then report that you have granted access to
	TwitVim and display a numeric PIN. Copy the PIN and paste it to the
	TwitVim input prompt "Enter OAuth PIN:".
...
3.1. TwitVim and OAuth					*TwitVim-OAuth*
	...
	After you log into Twitter/identi.ca with OAuth, TwitVim stores the
	OAuth access token in a file so that you won't have to log in again
	when you restart TwitVim. By default, this file is
	$HOME/.twitvim.token
...
<
	Since the access token grants full access to your Twitter/identi.ca
	account, it is recommended that you place the token file in a
	directory that is not readable or accessible by other users.
...
<
	...
	If TwitVim is logged in and you need to log in as a
	different Twitter/identi.ca user, visit Twitter/identi.ca, sign out
	and then sign in as the other user. Then use |:SetLoginTwitter| to
...
3.2. StatusNet						*TwitVim-StatusNet*
	...
	Note: This section is for StatusNet services other than identi.ca. If
	you are connecting to identi.ca, you can do so via OAuth using
	|:SetLoginTwitter|.

	StatusNet offers a Twitter-compatible API so you can use TwitVim with
	StatusNet. Setting up TwitVim for StatusNet is a bit different from
	setting up TwitVim for Twitter because StatusNet uses Basic
...
<
	Replace USER with your user name and PASS with your password.
	...
	servers other than Twitter and identi.ca that implement a
	Twitter-compatible API.

	For example, add the following to your vimrc:
>
		let twitvim_api_root = "https://statusnet-server.com/api"
<
	You'll have to obtain the actual API root from the service's
	documentation.

	And now you are ready to use TwitVim with StatusNet.
...
<
	Note: This is only for StatusNet services that use Basic
	authentication. TwitVim connects to Twitter and identi.ca using OAuth,
	so you do not need to add login info to your vimrc.
...
------------------------------------------------------------------------------
3.5. Using Twitter/identi.ca SSL API			*TwitVim-ssl*

							*twitvim_force_ssl*
	For added security, you can make TwitVim use SSL when calling the
	Twitter/identi.ca API by adding the following to your vimrc:
>
...
<
	...
	Use this command if you need to log in as another Twitter or identi.ca
	user. When the authentication web page comes up, use the "Sign Out"
	link to log in as a different user and grant TwitVim access to that
	user.
	
	Note: On identi.ca, you may have to sign out at their website prior to
	initiating :SetLoginTwitter in order to switch users.
	...
	This command has no effect in StatusNet services that use Basic
	authentication.
...
<
	...
	out of Twitter for hitting the rate limit. This command does not work
	on identi.ca.
...
------------------------------------------------------------------------------
6.2. Switching between StatusNet users			*TwitVim-switch*

	If you have multiple user accounts on StatusNet, you can add something
	like to following to your vimrc to make it easy to switch between
	those accounts:
>
		function! Switch_to_statusnet_user1()
		    let g:twitvim_api_root = "https://statusnet-server.com/api"
		...
		function! Switch_to_statusnet_user2()
		    let g:twitvim_api_root = "https://statusnet-server.com/api"
		...
		command! ToStatusNet1 :call Switch_to_statusnet_user1()
		command! ToStatusNet2 :call Switch_to_statusnet_user2()
<	
	With that in place, you can use :ToStatusNet1 and :ToStatusNet2 to
	...
	Note: This is not needed on Twitter and identi.ca because login
	sessions are managed via OAuth. Use |:SwitchLoginTwitter| instead to
